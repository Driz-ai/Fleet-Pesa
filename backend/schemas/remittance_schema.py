...
class RemittanceCreateSchema(Schema):
    ...
    # expected_amount and status are intentionally absent: expected_amount is copied from
    # vehicle.daily_expected_amount at submission time, and status is computed server-side
    # from actual vs expected. Never accepted from the client (per ERD note).
